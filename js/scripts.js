...
$(document).ready(function(){
	...
	if($('#homeslider')[0]){
		...
		$('.nav > li')[clickEvent]( function() {
			if(windowWidth > 999){
				$('.megamenu').css('top', $('.nav').height()).stop(true, true).fadeIn(300);
				$(this).siblings().removeClass('sub_opened');
				$(this).toggleClass('sub_opened');
				if($(this).hasClass('sub_opened')){
					$(".megamenu_wrap").css({'opacity': '0'});
					$('.megamenu').css({'z-index': '0'});
					$(this).find('.megamenu').css({'z-index': '1000'});
					$(this).find('.megamenu_wrap').css({'opacity': '1'});
				}
				else{
					$(this).find('.megamenu').stop(true, true).fadeOut(300);
				}
				return false;
			}
		});
